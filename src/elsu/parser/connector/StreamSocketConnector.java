...
public class StreamSocketConnector extends ConnectorBase {
	...
	public StreamSocketConnector(ConfigLoader config, String connName, String host, int port, String name, String id)
			throws Exception {
		super();
		...
		if (connName != null) {
			...
			noDataTimeout = Integer.parseInt(config
					.getProperty("application.services.service." + connName + ".attributes.key.monitor.noDataTimeout")
					.toString());
			retryWaitTime = Integer.parseInt(config
					.getProperty("application.services.service." + connName + ".attributes.key.monitor.idleTimeout")
					.toString());
			localStoreMask = config
					.getProperty("application.services.service." + connName + ".attributes.key.localStore.mask")
					.toString();
			localStorePath = config
					.getProperty("application.services.service." + connName + ".attributes.key.localStore.directory")
					.toString();
			rolloverFrequency = Integer.parseInt(config
					.getProperty("application.services.service." + connName + ".attributes.key.log.rollover.frequency")
					.toString());
			rolloverPeriodicity = FileRolloverPeriodicityType.valueOf(config
					.getProperty(
							"application.services.service." + connName + ".attributes.key.log.rollover.periodicity")
					.toString());
			siteId = config.getProperty("application.services.service." + connName + ".attributes.key.site.id")
					.toString();
		...
		System.out.println(getClass().toString() + ", StreamNetworkConnector(), " + "client config loaded, "
				...
				+ ", " + "retryWaitTime: " + retryWaitTime + ", " + "localStoreMask: " + localStoreMask + ", "
				+ "localStorePath: " + localStorePath + ", " + "rolloverFrequency: " + rolloverFrequency + ", "
				+ "rolloverPeriodicity: " + rolloverPeriodicity + ", " + "siteId: " + siteId + ", " + "siteName: "
				+ siteName);
		...
		messageWriter = new FileChannelTextWriter(String.format(fileMask, "%s", "MSG"), localStorePath + "incomming/",
				rolloverPeriodicity);
