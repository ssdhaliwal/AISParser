...
public class T6_DangerousCargo extends T6_BinaryAddressedMessage {
	...
	public String toString() {
		...
		try {
			...
			node.put("dac", getDac());
			node.put("fid", getFid());
			node.put("lastPortOfCall", getLastPortOfCall());
			node.put("lpocETAMonth", getLpocETAMonth());
			node.put("lpocETADay", getLpocETADay());
			node.put("lpocETAHour", getLpocETAHour());
			node.put("lpocETAMinute", getLpocETAMinute());
			node.put("nextPortOfCall", getNextPortOfCall());
			node.put("npocETAMonth", getNpocETAMonth());
			node.put("npocETADay", getNpocETADay());
			node.put("npocETAHour", getNpocETAHour());
			node.put("npocETAMinute", getNpocETAMinute());
			node.put("dangerousCargo", getDangerousCargo());
			node.put("imdCategory", getImdCategory());
			node.put("UNNumber", getUNNumber());
			node.put("amount", getAmount());
			node.put("unitOfQuantity", getUnitOfQuantity());
			node.put("unitOfQuantityText", getUnitOfQuantity());
